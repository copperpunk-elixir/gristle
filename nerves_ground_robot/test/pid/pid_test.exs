defmodule Pid.PidTest do
  ...
  assert_in_delta(Pid.Pid.get_cmd_for_error(:roll, 0, 0, 1), current_cmd, compare_delta)
  cmd_error = 0.01
  rate_act = 1.0
  dt = 1.0
  # Update PID
  Pid.Pid.get_cmd_for_error(:roll, cmd_error, rate_act, dt)
  expected_cmd = Common.Utils.Math.constrain(((cmd_error*pid_config.kp) - rate_act)*pid_config.kd + 0.5, 0,1)
  assert_in_delta(Pid.Pid.get_last_cmd(:roll), expected_cmd, compare_delta)

  # Test 2 - Several PID updates in a row
  pid_config_2 = %{pid_config | name: :pitch, rate_or_position: :position}
  Pid.Pid.start_link(pid_config_2)
  num_steps = 20
  Enum.each(1..num_steps, fn _ ->
    Pid.Pid.get_cmd_for_error(:pitch, cmd_error, rate_act, dt)
  end)
  ...
  assert_in_delta(Pid.Pid.get_last_cmd(:pitch), expected_cmd, 0.00001)
