defmodule Peripherals.I2c.Adsadc do
  ...
  def new_adsadc(config) do
    ...
    %Peripherals.I2c.Adsadc{bus_ref: bus_ref, address: address}
  end

  # def read_all_channels(%Peripherals.I2c.Adsadc{bus_ref: bus_ref, address: address, channels: channels}) do
  ...
  defp read_channel(bus_ref, address, pin) do
    ...
    Process.sleep(1)
    value = Peripherals.I2c.Utils.write_read(bus_ref, address, @pointer_convert, 2)
    if value == [] do
      {:error, :bad_ack}
    else
