defmodule Peripherals.Uart.Bno080 do
  ...
  # defstruct bus_ref: nil, address: nil,
    ...
    # shtp_header: {0,0,0,0},
   # accel: %{accel: {0,0,0}, status: 0, delay_us: 0},

   # update_interval: nil

  # @default_bus "i2c-3"
  # @default_address 0x4B
  ...
  # def spawn_loop(state) do
  #   spawn(fn -> data_loop(state) end)
  # end
  ...
  defp hard_reset(wake_ref, reset_ref) do
    ...
    Process.sleep(1000)
    Circuits.GPIO.write(reset_ref, 1)
    Process.sleep(@default_sleep)
    Circuits.GPIO.write(wake_ref,0)
  end
  ...
  # defp build_report_string(state, report_id) do
  #   {data_string, data_field} = case report_id do
  #     @sensor_reportid_accelerometer ->
  #       {"Accel X/Y/Z: #{inspect(state.accel.accel)},", :accel}
  #     @sensor_reportid_rotation_vector ->
  #       euler_deg = Common.Utils.rad2deg_with_length(state.attitude.euler, 3)
  #       {"Rot. Vec RPY: #{inspect(euler_deg)}", :attitude}
  #     @sensor_reportid_arvr_game_rotation_vector ->
  #       euler_deg = Common.Utils.rad2deg_with_length(state.attitude.euler, 3)
  #       {"AR Game Rot. Vec RPY: #{inspect(euler_deg)}", :quat}
  #     id ->
  #       {"Unknown message with id #{id}", :unknown}
  #   end
  #   data_string# <> build_status_time_string(data_field, state)
  # end

  # defp build_status_time_string(state, data_field) do
  #   data = Map.fetch!(state, data_field)
  #   ", status: #{Map.fetch!(data, :status)}, delay_us: #{Map.fetch!(data, :delay_us)}"
  # end
