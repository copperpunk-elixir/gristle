defmodule Pids.Steering.Plane do
  ...
  def calculate_outputs(cmds, values, airspeed, dt) do
    ...
    # Logger.debug("course cmds: #{inspect(cmds)}")
      yaw_rotate_cmd = cmds.course_rotate# Pids.Pid.update_pid(:course_rotate, :yaw, cmds.course_rotate, 0.0, airspeed, dt)
      # Logger.debug("yawrot/crsrot: #{Common.Utils.eftb_deg(yaw_rotate_cmd,1)}/#{Common.Utils.eftb_deg(cmds.course_rotate,1)}")
    # else
