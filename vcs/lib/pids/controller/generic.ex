defmodule Pids.Controller.Generic do
  ...
  def begin(config) do
    ...
    delta_output_min =
      case config[:delta_output_min] do
        nil -> -output_mid
        value -> value
      end
    delta_output_max =
      case config[:delta_output_max] do
        nil -> output_mid
        value -> value
      end
    %{
  ...
  def update(pv_cmd, pv_value, airspeed, dt, state) do
    ...
    cmd_d = if dt != 0, do: -state.kd*(correction- state.pv_correction_prev)/dt, else: 0
    ...
    |> Common.Utils.Math.constrain(state.delta_output_min, state.delta_output_max)
    ...
    %{state | output: output,  pv_correction_prev: correction, pv_integrator: pv_integrator}
  end
