defmodule Navigation.Path.CreatePathFollowerTest do
  ...
  test "Create Path Follower" do
    ...
    pc = Enum.at(pcs, pci)
    # Starting at wp1
    {speed, course, alt} = Navigation.Path.PathFollower.follow(pf, wp1, :math.pi/2, pc)
    # Move in the positive Y direction
    {lat,lon} = Common.Utils.Location.lat_lon_from_point(wp1, 0, 2)
    {speed, course, alt} = Navigation.Path.PathFollower.follow(pf, Navigation.Path.LatLonAlt.new(lat,lon), -0.1 + :math.pi/2, pc)
    ...
    {speed, course, alt} = Navigation.Path.PathFollower.follow(pf, Navigation.Path.LatLonAlt.new(lat,lon), :math.pi/4, pc)
