defmodule Actuation.HwInterfacePololuTest do
  ...
  test "Start HWInterface. Connect to Pololu Maestro. Change actuator values", context do
   ...
   Actuation.HwInterface.set_actuator_output(aileron, aileron.cmd_limit_min)
   assert Actuation.HwInterface.get_actuator_output(aileron) == aileron.min_pw_ms
   Process.sleep(100)
   # Set output to max value
   Actuation.HwInterface.set_actuator_output(aileron, aileron.cmd_limit_max)
   assert Actuation.HwInterface.get_actuator_output(aileron) == aileron.max_pw_ms
   Process.sleep(100)
   # Set output to neutral value
   Actuation.HwInterface.set_actuator_output(aileron, 0.5*(aileron.cmd_limit_min + aileron.cmd_limit_max))
   assert Actuation.HwInterface.get_actuator_output(aileron) == 0.5*(aileron.min_pw_ms + aileron.max_pw_ms)
   Process.sleep(100)
